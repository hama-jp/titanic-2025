"""
Titanic Perished Prediction - Optuna Focused on Shallow Trees
v2.6ベース + 浅い木に絞った探索 + アーリーストッピング最適化

戦略:
1. v2.6の特徴量セット（16個）を使用
2. 浅い木に絞って探索:
   - num_leaves: 8 ~ 24（v2.6の16を中心に）
   - max_depth: 3 ~ 5（v2.6の4を中心に）
3. アーリーストッピングも最適化対象に追加
4. 150 trials で探索（前回より多く）
"""
...
# ====================================================================
# 2. Optuna目的関数（浅い木に絞った探索）
# ====================================================================

def objective(trial, X, y, features, categorical_features, n_splits=5, seed=42):
    """Optunaの目的関数：浅い木に絞って探索"""

    # ハイパーパラメータの探索空間（浅い木に絞る）
    params = {
        'objective': 'binary',
        'metric': 'binary_logloss',
        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
        'num_leaves': trial.suggest_int('num_leaves', 8, 24),  # 狭く（前回: 8-64）
        'max_depth': trial.suggest_int('max_depth', 3, 5),      # 狭く（前回: 3-10）
        'min_data_in_leaf': trial.suggest_int('min_data_in_leaf', 15, 35),  # v2.6の20を中心に
...
if __name__ == "__main__":
    print("=" * 60)
    print("Titanic - Optuna Shallow Trees Optimization")
    print("浅い木に絞った探索 + アーリーストッピング最適化")
    print("=" * 60)
    print()
    ...
    print("=" * 60)
    print("探索回数: 150 trials")
    print("探索範囲（浅い木に絞る）:")
    print("  - num_leaves: 8-24 (v2.6: 16)")
    print("  - max_depth: 3-5 (v2.6: 4)")
    print("  - early_stopping_rounds: 50-150 (NEW!)")
    ...
    for key, value in study.best_params.items():
        if key == 'num_leaves':
            print(f"  {key}: {value} (v2.6: 16)")
        elif key == 'max_depth':
            print(f"  {key}: {value} (v2.6: 4)")
        elif key == 'early_stopping_rounds':
            print(f"  {key}: {value} (v2.6: 100)")
        else:
            print(f"  {key}: {value}")
    print()
    ...
    submission_file = 'submission_optuna_shallow.csv'
    ...
    print(f"Optuna浅い木版: {study.best_value:.2%} OOF Accuracy")

...
### 推奨1: **v2.2**（最もバランスが良い）⭐ **NEW**

**理由**:
- **OOF Accuracy: 84.29%**（v2系で最高、v1に迫る）
- **OOF AUC: 88.52%**（v1より高い）
- **特徴量数: 17個**（v2より洗練、冗長性削減）
- train+testグループ情報を活用しつつ、完全冗長な特徴を削除

**使用ケース**:
- 最もバランスの取れたモデルが欲しい場合
- 冗長性を排除したクリーンなモデルが望ましい場合
...
| `submission_restart_v2_2.csv` | **v2.2** | **84.29%** | 62.9% |
...
**v2.2（OOF Acc: 84.29%）を第一推奨、v1（OOF Acc: 84.96%）を第二推奨**

- **v2.2**: 最もバランスが良い（Accuracy, AUC, 特徴量数すべて優秀）
  - train+test情報を活用しつつ、冗長性を排除
  - 17特徴量でクリーンなモデル構造
- **v1**: 最高のAccuracy、最もシンプル、最も安定
  - 過学習リスクが最も低い
  - 16特徴量で解釈性が高い

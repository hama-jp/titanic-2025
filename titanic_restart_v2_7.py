...
def make_features(train_raw, test_raw):
    ...
    # I. Age欠損補完（Title・Pclass・Sex別の中央値）
    print("欠損値補完: Age (Title + Pclass + Sex別)")
    ...
    for df in [train, test]:
        ...
        for idx in df[mask_missing].index:
            ...
            if (title, pclass, sex) in age_by_title_pclass_sex.index:
                df.loc[idx, 'Age'] = age_by_title_pclass_sex[(title, pclass, sex)]
            # 見つからなければTitle + Pclassで補完
            elif (title, pclass) in train.groupby(['Title', 'Pclass'])['Age'].median().index:
                df.loc[idx, 'Age'] = train.groupby(['Title', 'Pclass'])['Age'].median()[(title, pclass)]
            # それでも見つからなければTitleのみで補完
            else:
                df.loc[idx, 'Age'] = train[train['Title'] == title]['Age'].median()

        # 最終的に残った欠損値は全体の中央値で補完
...
if __name__ == "__main__":
    ...
    # 使用する特徴量（v2.7: Age復活 with 改良補完）
    features = [
        # 数値
        'Age',  # v2.7: Title+Pclass+Sex別補完で復活
        'Fare', 'FarePerPerson',
    ...
    print("v2.7の変更:")
    print("  - v2.6からAge を復活（改良補完: Title+Pclass+Sex別）")
    print("  - 理由: より精密な欠損値補完でAgeのノイズを削減")
    print("  - 特徴量数: 16個 → 17個")
    print()
    ...
    submission_file = 'submission_restart_v2_7.csv'
    ...
    print("=" * 60)
    print("✓ v2.6から変更:")
    print("  - Age を復活（改良補完: Title+Pclass+Sex別）")
    print("  - 特徴量数: 16個 → 17個")
    print()
    print("検証ポイント:")
    print("  1. v2.6とのCVスコア比較（85.19% vs ?）")
    print("  2. 改良補完でAgeのノイズが削減されるか")
    print("=" * 60)
    print("完了！")
    print("=" * 60)
